...
| `DataGrid`/`Column` | `DataGrid<T>`/`DataGridColumn<T>` | ✅ | Typed, self-rendering `: Decorator` (generics can't host `StyledProperty`/`ControlTheme`): sort headers, `PageSize` paging, striping/hover/`Dense`, single-row selection, `FilterText`/`Filter`, `Virtualize`/`MaxRenderedRows`, editable text cells via `SetText`, custom `CellTemplate`. Pure `DataGrids.Sort`/`PageCount`/`Filter`/`Group`. **v3 (Phase 5):** `GroupBy` selector renders group-header rows (key + count) in first-appearance order; `CollapsibleGroups` (default on) makes headers click/keyboard toggles that show/hide a group's rows (collapsed state keyed by group key); `EmptyText`/`EmptyContent` empty state when no rows; `DataGridColumn<T>.Width` (pixel/star sizing); `FrozenColumns` two-pane layout (pinned left grid + horizontally-scrollable rest, synced hover/selection via `RowVisual`, `RowHeight` for alignment; falls back to single grid while grouped); `GroupAggregate` text in group headers. Richer inline-edit (validation/commit) remains future expansion. |
...
| `NavigationRail` (M3) | `NavigationRail`/`NavigationRailItem` | ✅ | **v3 (Phase 4):** compact vertical destination rail (`: Decorator`, self-composed). `NavigationRailItem` (icon-over-label, secondary-container active-indicator pill, hover/focus state layer, click + keyboard activation). `NavigationRail` (`Items`, optional `Header`, two-way `SelectedIndex`, `SelectedItem`, `SelectionChanged`). |
| `BottomNavigation` (M3) | `BottomNavigation`/`BottomNavigationItem` | ✅ | **v3 (Phase 4):** horizontal bottom bar, equal-width cells (`: Decorator` + `UniformGrid`), surface-container background. `BottomNavigationItem : NavigationRailItem` (shared item anatomy). Two-way `SelectedIndex`, `SelectedItem`, `SelectionChanged`. |
| `CommandPalette` (M3-style) | `CommandPalette`/`CommandPaletteItem` | ✅ | **v3 (Phase 4):** searchable command list — search `TextField` + live-filtered `ListItem` rows on an elevated `Paper`; keyboard Down/Up/Enter/Escape; pure testable `Filter(commands, query)`; two-way `FilterText`/`IsOpen`; `Invoked`/`Closed`. Overlay-service hosting ⬜. |
