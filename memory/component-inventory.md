...
| `BottomNavigation` (M3) | `BottomNavigation`/`BottomNavigationItem` | ✅ | **v3 (Phase 4):** horizontal bottom bar, equal-width cells (`: Decorator` + `UniformGrid`), surface-container background. `BottomNavigationItem : NavigationRailItem` (shared item anatomy). Two-way `SelectedIndex`, `SelectedItem`, `SelectionChanged`. `CommandPalette` ⬜. |
