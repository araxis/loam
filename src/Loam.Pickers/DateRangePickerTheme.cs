...
/// <summary>Builds the <see cref="DateRangePicker"/> theme: an optional label over a variant field (<c>PART_Box</c>) showing the selected range (<c>PART_Display</c>) and a calendar icon.</summary>
internal static class DateRangePickerTheme
{
    public static ControlTheme Create() =>
        new(typeof(DateRangePicker))
        {
            Setters = { new Setter(TemplatedControl.TemplateProperty, BuildTemplate()) },
        };

    private static FuncControlTemplate<DateRangePicker> BuildTemplate() =>
        new((picker, scope) =>
        {
            var label = new Text
            {
