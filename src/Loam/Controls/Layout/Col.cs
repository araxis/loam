...
/// <summary>
/// A child of <see cref="ResponsiveGrid"/> that spans a number of the 12 columns per breakpoint. Holds
/// a single <see cref="Avalonia.Controls.Decorator.Child"/>. A span resolves to the value at the
/// current breakpoint or the nearest smaller one that is set; unset defaults to a full row (12).
/// </summary>
/// <remarks>
/// Renamed from <c>Item</c> in Loam v3. The old <see cref="Item"/> name remains as a deprecated alias.
/// </remarks>
public class Col : Decorator
{
    /// <summary>Identifies the <see cref="Xs"/> property.</summary>
    public static readonly StyledProperty<int> XsProperty = AvaloniaProperty.Register<Col, int>(nameof(Xs));

    /// <summary>Identifies the <see cref="Sm"/> property.</summary>
    public static readonly StyledProperty<int> SmProperty = AvaloniaProperty.Register<Col, int>(nameof(Sm));

    /// <summary>Identifies the <see cref="Md"/> property.</summary>
    public static readonly StyledProperty<int> MdProperty = AvaloniaProperty.Register<Col, int>(nameof(Md));

    /// <summary>Identifies the <see cref="Lg"/> property.</summary>
    public static readonly StyledProperty<int> LgProperty = AvaloniaProperty.Register<Col, int>(nameof(Lg));

    /// <summary>Identifies the <see cref="Xl"/> property.</summary>
    public static readonly StyledProperty<int> XlProperty = AvaloniaProperty.Register<Col, int>(nameof(Xl));

    /// <summary>Identifies the <see cref="Xxl"/> property.</summary>
    public static readonly StyledProperty<int> XxlProperty = AvaloniaProperty.Register<Col, int>(nameof(Xxl));

    static Col() =>
        // Invalidating the column's measure bubbles up to re-run the parent ResponsiveGrid's layout.
        AffectsMeasure<Col>(XsProperty, SmProperty, MdProperty, LgProperty, XlProperty, XxlProperty);

    /// <summary>Creates the grid column.</summary>
    public Col() => AutomationProperties.SetName(this, "Grid item");
